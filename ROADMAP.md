...
**Outcome:** `SplashScreen` (`features/splash/presentation/screens/`) shows a fade+scale branded entrance (`CircleAvatar` + `Icons.eco` + app name/tagline, all theme-driven) using new `core/theme/app_motion.dart` (`AppDurations`/`AppCurves`, per `UI_GUIDELINES.md` §5), waits for a minimum display duration concurrently with a `CheckOnboardingStatus` use-case call, then routes via `go_router`'s `context.go`. The `onboarding` feature is fully layered per `PROJECT_SPEC.md` §3: `OnboardingLocalDataSource` (`shared_preferences`, key `onboarding_complete`) → `OnboardingRepositoryImpl` → `CheckOnboardingStatus`/`CompleteOnboarding` use cases → `OnboardingProvider` (page index + completion) driving a 4-slide `OnboardingScreen` (`PageView`, animated dot indicator built as `OnboardingPageIndicator`, Skip/Next/Get Started controls). `core/routing/app_router.dart` gained `/splash` (new `initialLocation`) and `/onboarding` routes. `main.dart` is now `async`, loading a single `SharedPreferences` instance before `runApp`; `app.dart` constructs the onboarding repository/use-cases and exposes them via the existing `MultiProvider` alongside `ThemeModeController`.

`flutter analyze` clean. `flutter test` passes (2 widget tests: fresh install routes to Onboarding, returning user routes to Home) — writing these surfaced two test-infrastructure gotchas worth remembering for future tests, neither an app bug: the `shared_preferences` plugin memoizes its singleton the first time `getInstance()` is called in a test process, so a shared instance must be mutated directly (not re-mocked) between tests; and `appRouter` is a process-wide top-level singleton, so each test must reset its location (`appRouter.go(AppRoutes.splash)`) to simulate a fresh launch. Live-verified on the Android emulator (Pixel_7) in both light and dark themes: fresh install pages through all 4 onboarding slides (swipe navigation confirmed working; note `adb shell input tap` needed carefully recomputed coordinates — an adb/testing artifact, not an app issue), Skip completes onboarding and lands on Home; after `pm clear` + cold relaunch, the returning-user path goes straight to Home, confirming the flag persists via real device `SharedPreferences`. The Splash screen's own frame wasn't caught in a screenshot (its ~1.1s window is narrower than the adb screenshot round-trip on this machine) but its correctness is covered by the widget tests plus the observed correct hand-off to both destinations in both themes.
